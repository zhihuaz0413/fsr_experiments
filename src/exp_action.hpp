...
class ExpAction {
 public:
  ExpAction() = default;
  explicit ExpAction(const ObjectConf& object) {
    name_ = object.object_name();
  ...
  bool GeneratePressingTraj(std::vector<std::unique_ptr<geometry_msgs::msg::Twist>>& twist, int idx) {
    int num_samples = duration_[idx] * sample_rate_;
    double t = 0.0;
  ...
  bool GeneratePrecisionTraj(std::vector<std::unique_ptr<geometry_msgs::msg::Twist>>& twist, int idx) {
    double period = duration_[idx] * 1.5;
    int num_samples = period * sample_rate_;
    double t = 0.0;
    t_.resize(num_samples);
    twist.resize(num_samples);
    ...
    for (int i = 0; i < num_samples; i++) {
      ...
      twist[i]->angular.x = angle_[idx] * 2.0 * M_PI / period * std::cos(2.0 * M_PI * t / period);
      if (i > num_samples / 6 && i < num_samples / 6 * 5) {
        twist[i]->angular.y = angle_[idx] * 2.0 * M_PI / period * std::cos(2.0 * M_PI * t / period - M_PI / 2);
  ...
  bool GenerateSlipperyTraj(std::vector<std::unique_ptr<geometry_msgs::msg::Twist>>& twist, int idx) {
    ...
    for (int i = 0; i < num_samples; i++) {
      t_[i] = t;
      twist[i] = std::make_unique<geometry_msgs::msg::Twist>();
      slip_x[i] = slip_amp_[idx] * std::sin(2.0 * M_PI * t / period);
      twist[i]->linear.x = slip_amp_[idx] * 2.0 * M_PI / period * std::cos(2.0 * M_PI * t / period);
      t += dt_;
    }
    for (int i = 0; i < num_samples; i++) {
      t_[i + num_samples] = t;
      twist[i + num_samples] = std::make_unique<geometry_msgs::msg::Twist>();
      slip_y[i] = slip_amp_[idx] * std::sin(2.0 * M_PI * t / period);
      twist[i + num_samples]->linear.y = slip_amp_[idx] * 2.0 * M_PI / period * std::cos(2.0 * M_PI * t / period);
      t += dt_;
    }
    for (int i = 0; i < num_samples; i++) {
      t_[i + num_samples * 2] = t;
      twist[i + num_samples * 2] = std::make_unique<geometry_msgs::msg::Twist>();
      slip_theta[i] = slip_angle_ * std::sin(2.0 * M_PI * t / period);
      twist[i + num_samples * 2]->angular.z = slip_angle_ * 2.0 * M_PI / period * std::cos(2.0 * M_PI * t / period);
      t += dt_;
    }
    return true;
